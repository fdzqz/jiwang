...
function search() {
    $.ajax({
        ...
        type:'post',
        data:{
            /*"searchkey":"$('input[name='field_k'] option:selected').val()",*/
            "searchkey":"$('input[name='field_k'] option:selected').val()",
            "inputvalue":"$('.inp')",
            "agencyname":"$('input[name='agent_id'] option:selected').text()",
...
$.ajax({
    ...
    success:function (data) {
        ...
        $.each(data.datas,function (index,comment) {
        	...
        	//格式化时间
        	var regdate = new Date(comment.regdate).format("yyyy-MM-dd HH:mm:ss");
